...
class GaltonBoard:
    ...
    def calculate_bin_index(self) -> int:
        ...
        for row in range(self.num_rows):
            ...
            distance_from_center: float = (position - peg_center) / self.PEG_RADIUS
            bounce_probability: float = max(0.2, min(0.8, 0.5 + (0.1 * distance_from_center)))
            direction: int = 1 if random() < bounce_probability else -1
            bounce_force: float = (1.0 - abs(distance_from_center)) * self.elasticity
...
def generate_galton_board() -> None:
    board = GaltonBoard(board_width=667, board_height=667, num_rows=12, num_balls=100_000)
    board.simulate()
    board.save_image()
